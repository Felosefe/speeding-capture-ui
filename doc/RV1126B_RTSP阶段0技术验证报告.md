...
状态：**工具已就绪，真机验证待执行，阶段 0 尚未完成**  
负责人：软件 C  
基准环境：Windows、Qt 6.11.1、MinGW 64 位  
首选候选：Qt Multimedia / FFmpeg backend
...
这些交付只证明 spike 可构建和重复执行，不等同于真实码流已经验收。
...
| 场景 | 执行方法 | 通过标准 | 实际结果 |
| --- | --- | --- | --- |
| 主码流连续播放 | `/live/0` 运行 30 分钟 | 无崩溃、永久卡流或持续资源增长 | 待执行 |
| 辅码流连续播放 | `/live/1` 运行 30 分钟 | 无崩溃、永久卡流或持续资源增长 | 待执行 |
| 主码流冷启动 | 点击“冷启动/重新打开”共 10 次 | 至少 9 次首帧不超过 3000 ms | 待执行 |
| 辅码流冷启动 | 点击“冷启动/重新打开”共 10 次 | 至少 9 次首帧不超过 3000 ms | 待执行 |
| 断网恢复 | 标记断网、实际断网、恢复并标记，共 5 次 | 状态进入重连，恢复后 30 秒内重新出帧 | 待执行 |
| 切流和停止 | 主辅切换、停止、重开、关闭程序 | 停止调用不超过 2000 ms，无 UI 卡死或残留进程 | 待执行 |
| HTTP 并行 | 全程配置 health URL | 视频故障不停止 health；HTTP 故障不停止视频 | 待执行 |
| 延迟和资源 | 每路至少记录 10 个延迟样本并保留 metrics 行 | 报告平均/峰值 CPU、内存、延迟 P50/P95 | 待执行 |
...
| 实际编码、分辨率、帧率 | 待填写 | 待填写 |
| 冷启动成功数/10 | 待填写 | 待填写 |
| 首帧 P50/P95/最大值 | 待填写 | 待填写 |
| 延迟 P50/P95 | 待填写 | 待填写 |
| CPU 平均/峰值 | 待填写 | 待填写 |
| 工作集平均/峰值 | 待填写 | 待填写 |
| 断网恢复成功数/5 | 待填写 | 待填写 |

只有上表和验证矩阵全部通过，才能把 Qt Multimedia/FFmpeg 冻结为正式后端。出现无法解码、
不可控重连、停止挂起或资源泄漏时，当天登记技术阻塞，并用外部 FFmpeg/libav 运行完全相同的矩阵。
...
| RTSP 真机矩阵通过且后端选型已冻结 | C | 待执行 |
| 联调板、Token、两路 RTSP、事件源持续可用 | A/B/C | 待确认 |

任一项未完成时，阶段 0 保持未完成；不得仅凭 spike 程序成功编译进入正式 UI 集成。
