...
BoardDeviceSession::BoardDeviceSession(
    ...
    , profileUpdater_(std::move(profileUpdater))
{
    snapshot_.profile = std::move(profile);
    retryTimer_.setSingleShot(true);
    connect(&retryTimer_, &QTimer::timeout, this, [this] { startHealthCheck(); });
}
...
{
    if (profile.deviceId.isEmpty() || profile.deviceId != snapshot_.profile.deviceId
